...
import { FiMenu, FiX, FiUser, FiShoppingCart, FiLogOut, FiLayout, FiSettings, FiTrash2 } from "react-icons/fi";
...
export default function Navbar() {
  ...
  const getLinkStyle = (path) =>
    `text-sm font-medium transition-colors duration-200 ${
      pathname === path ? "text-blue-600" : "text-slate-600 hover:text-slate-900"
    }`;

  const getMobileLinkStyle = (path) =>
    `block px-4 py-3 rounded-lg font-medium text-base transition-colors ${
      pathname === path ? "bg-blue-50 text-blue-600" : "text-slate-700 hover:bg-slate-50"
    }`;
    ...
    <nav className="sticky top-0 z-50 bg-white border-b border-slate-100 shadow-sm">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <Link href="/categories" className={getLinkStyle("/categories")}>Categories</Link>
          ...
          <div className="flex items-center gap-3">
              ...
              {cartOpen && (
                <div className="hidden lg:block absolute right-0 mt-2 w-96 rounded-xl border border-slate-100 bg-white shadow-xl overflow-hidden z-50">
                  <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
                    <p className="font-semibold text-slate-800 text-sm">Shopping Cart ({cartItems.length})</p>
                  </div>
                  ...
                  <div className="max-h-72 overflow-y-auto p-3 space-y-3">
                    {cartItems.length === 0 ? (
                      ...
                      cartItems.map((item) => (
                        <div key={item._id} className="flex items-center gap-3 p-2 rounded-xl border border-slate-50 hover:bg-slate-50/50 transition">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-blue-600 font-bold mt-0.5">৳{item.price}</p>
                          </div>
                  ...
                  {cartItems.length > 0 && (
                    <div className="p-3 border-t border-slate-100 bg-slate-50/50">
                      <div className="flex justify-between text-xs font-bold text-slate-700 mb-2.5 px-1">
                        <span>Total:</span>
                        <span className="text-blue-600 text-sm">৳{totalPrice}</span>
                      </div>
                      <Link
                        href="/checkout"
            ...
            {!user ? (
              <div className="hidden sm:flex items-center gap-2">
                <Link href="/login" className="px-3 py-1.5 text-sm font-medium text-slate-700 hover:text-slate-900">
                  Sign In
                </Link>
                <Link href="/register" className="px-3 py-1.5 text-sm font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                  Get Started
                </Link>
              </div>
            ) : (
      ...
      {mobileMenu && (
        <div className="md:hidden border-t border-slate-100 bg-white px-4 py-3 space-y-1 shadow-inner">
          <Link href="/" onClick={() => setMobileMenu(false)} className={getMobileLinkStyle("/")}>Home</Link>
          <Link href="/products" onClick={() => setMobileMenu(false)} className={getMobileLinkStyle("/products")}>Products</Link>
          <Link href="/categories" onClick={() => setMobileMenu(false)} className={getMobileLinkStyle("/categories")}>Categories</Link>
          <Link href="/about" onClick={() => setMobileMenu(false)} className={getMobileLinkStyle("/about")}>About</Link>
          <Link href="/contact" onClick={() => setMobileMenu(false)} className={getMobileLinkStyle("/contact")}>Contact</Link>
          
          {!user && (
            <div className="pt-4 mt-2 border-t border-slate-100 grid grid-cols-2 gap-2">
              <Link href="/login" onClick={() => setMobileMenu(false)} className="block text-center py-2 text-sm font-medium text-slate-700 bg-slate-50 rounded-lg">
                Sign In
              </Link>
              <Link href="/register" onClick={() => setMobileMenu(false)} className="block text-center py-2 text-sm font-medium text-white bg-blue-600 rounded-lg">
                Sign Up
              </Link>
            </div>
          )}
