...
import { FiCheckCircle, FiAlertTriangle, FiShoppingBag, FiArrowRight, FiCalendar, FiActivity } from "react-icons/fi";
  ...
  useEffect(() => {
    ...
    const verifyPayment = async () => {
      try {
        ...
        const response = await axios.post(`${process.env.NEXT_PUBLIC_SERVER_URL}/verify-payment`, {
        ...
        setError(err.response?.data?.message || "Something went wrong while verifying payment.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white p-4">
        ...
        <h2 className="text-xl font-semibold text-slate-700 animate-pulse">Verifying Your Payment...</h2>
        <p className="text-sm text-slate-400 mt-1">Please do not close or refresh this window.</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white p-4">
        ...
        <h1 className="text-2xl font-bold text-slate-800">Payment Verification Failed</h1>
    ...
    <div className="min-h-screen bg-slate-50/50 py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
      <div className="max-w-xl w-full bg-white border border-slate-100 rounded-3xl shadow-xl shadow-slate-100/50 p-6 sm:p-8 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center text-center pb-6 border-b border-slate-100">
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-800">Payment Successful!</h1>
          <p className="text-sm text-slate-500 mt-1">Thank you for your purchase. Your order has been placed.</p>
        </div>

        {/* Order and Transaction Summary */}
        <div className="py-6 space-y-4">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Transaction Details</h2>
          ...
          {paymentData?.orders && paymentData.orders.length > 0 && (
            <div className="bg-slate-50 rounded-2xl p-4 sm:p-5 space-y-3 text-sm">
              <div className="flex justify-between items-center">
                <span className="text-slate-500 flex items-center gap-2"><FiActivity size={16}/> Transaction ID</span>
                <span className="font-mono font-bold text-slate-800 bg-white px-2 py-1 rounded-md border border-slate-200 text-xs">
                  {paymentData.orders[0].txnId} {/* ✅ Mapping txnId from the 1st object of the array */}
                </span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-slate-500 flex items-center gap-2"><FiCalendar size={16}/> Payment Date</span>
                <span className="font-semibold text-slate-700">
                  {paymentData.orders[0].createdAt} {/* ✅ Mapping 'createdAt' from database */}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-slate-700 bg-blue-50 text-blue-700 text-xs px-2.5 py-1 rounded-full">
                  {paymentData.orders[0].paymentLabel || "Stripe Secure Card"} {/* ✅ Mapping 'paymentLabel' from database */}
                </span>
              </div>
              ...
              <div className="border-t border-slate-200/60 pt-3 flex justify-between items-center text-base font-bold">
                ...
                <span className="text-blue-600 text-lg">
                  ৳{paymentData.amount || paymentData.orders.reduce((sum, item) => sum + Number(item.price), 0)} {/* ✅ Total amount calculation */}
                </span>
              </div>
            </div>
          )}
        ...
        <div className="pb-6">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Items Purchased</h3>
          <div className="max-h-36 overflow-y-auto space-y-2 pr-1">
            {paymentData?.orders?.map((item, index) => (
              <div key={index} className="flex justify-between items-center bg-slate-50/50 p-2.5 rounded-xl border border-slate-100 text-xs">
                <div className="min-w-0 flex-1 pr-3">
                  <p className="font-semibold text-slate-800 truncate">{item.productTitle}</p>
